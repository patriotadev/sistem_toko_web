...
function Main() {
  ...
  return (
    <>
      ...
      />
      <h2 className="mt-10 text-lg font-medium intro-y">Daftar Invoice - Penjualan</h2>
        ...
        <div className="flex flex-wrap items-center col-span-12 mt-2 intro-y sm:flex-nowrap gap-2">
         ...
         <div className="col-span-12 overflow-auto intro-y xl:overflow-visible ">
          {!isDataLoading ? <Table className="border-spacing-y-[10px] border-separate -mt-2">
            ...
            <Table.Tbody>
              {invoicePenjualanList && invoicePenjualanList.length > 0 ? invoicePenjualanList.map((item, index) => (
                <Table.Tr key={index} className="intro-x">
                  ...
                  <Table.Td className="first:rounded-l-md last:rounded-r-md text-start bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b]">
                    { item.status === Status.SEDANG_DIPROSES &&
                      <span className="bg-primary flex items-center justify-center rounded-full text-white text-xs w-fit py-1 px-2">
                        <Lucide icon="FileInput" className="w-4 h-4 mr-2" />
                        {item.status}
                      </span>
                    }
                    {
                      item.status === Status.JATUH_TEMPO && 
                      <span className="bg-danger flex items-center justify-center rounded-full text-white text-xs w-fit py-1 px-2">
                    ...
                    }
                    {
                      ...
                      <span className="bg-success flex items-center justify-center rounded-full text-white text-xs w-fit py-1 px-2">
